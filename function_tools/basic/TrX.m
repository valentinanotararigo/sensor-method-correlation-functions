...
%
% e.g. if the system is of the form rho = p_N kron p_M with subscripts denoting
% sizes, to trace over p_M's DOF one would take TrX(rho,2,[N,M])
...
if any(size(p) == 1)
  % state vector (don't feed this a Louiville space density matrix)
  if size(p,1) == 1
    p = p';
  end
